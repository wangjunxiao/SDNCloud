...
public class DateUtil {
	...
	/**
	 * compare if date is within from and to(includes from and to)
	 * @param date
	 * @param from
	 * @param to
	 * @return
	...
	public static String getMinBetweenDate(String expire){
		...
		try {
			...
			if (eDate.before(now)) {
				return "failed"+((now.getTime()-eDate.getTime())/1000/60);
			}
			if (eDate.after(now)) {
